...
public extension UIImage {
    ...
    func crop(pixelRect: CGRect) -> UIImage {
        ...
        let rect = CGRect.init(x: ceil(pixelRect.origin.x),
                               y: ceil(pixelRect.origin.y),
        ...
        UIRectFill(CGRect.init(origin: .zero, size: rect.size))
    ...
    func crop(aspectRatio: CGFloat) -> UIImage {
        ...
        if size.ratio == aspectRatio {
            return self
        }
        let rect = AVMakeRect(aspectRatio: CGSize.init(width: aspectRatio, height: 1), insideRect: CGRect.init(origin: .zero, size: CGSize.init(width: pixelWidth, height: pixelHeight)))
        return crop(pixelRect: rect)
    }
